$content = Get-Content 'd:\asiaze news\newssapp\lib\main.dart' -Raw
...
for($i=0; $i -lt $lines.Length; $i++) {
    $newLines += $lines[$i]
    if($lines[$i] -match '^\s+\}$' -and $i -gt 0 -and $lines[$i-1] -match 'Recently' -and $i -lt $lines.Length-1 -and $lines[$i+1] -match '@override') {
        $newLines += $handler
    }
}

$newLines -join "`r`n" | Set-Content 'd:\asiaze news\newssapp\lib\main.dart' -NoNewline
